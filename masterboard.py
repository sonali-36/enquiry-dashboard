...
# -----------------------------
SERVICE_ACCOUNT_FILE = r"C:\Users\Administrator\Desktop\Enquiry\credentials.json"  # or use relative path
...
df['Expected_Value'] = pd.to_numeric(
    df['Expected_Value'], errors='coerce'
).fillna(0)

df['Final_Order_Value'] = pd.to_numeric(
    df['Final_Order_Value'], errors='coerce'
).fillna(0)
...
sample_approved_df = df[
    (df['Sample_Status'] == 'approved')
]

order_confirmed_df = df[
    (df['Sample_Status'] == 'approved') &
    (df['Order_Confirmed'] == 'yes')
]
...
# -----------------------------
lead_to_sample_pct = (
    sample_approved_count / total_enquiries * 100
    if total_enquiries else 0
)

sample_to_order_pct = (
    order_confirmed_count / sample_approved_count * 100
    if sample_approved_count else 0
)

overall_conversion_pct = (
    order_confirmed_count / total_enquiries * 100
    if total_enquiries else 0
)
...
value_conversion_pct = (
    total_final / total_expected * 100
...
if 'Week' in df.columns:
    ...
    weekly_df['Conversion_%'] = (
        weekly_df['Orders'] / weekly_df['Enquiries'] * 100
    )
...
if 'Lead_Source' in df.columns:
    ...
    lead_df['Conversion_%'] = (
        lead_df['Orders'] / lead_df['Enquiries'] * 100
    )
